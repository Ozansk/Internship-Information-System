  ...
  db.query("SELECT holiday_date FROM holiday_list", (error, rows, results) => {
    ...
    */
    ...
    if (p2 == temp) {
      ...
      if (p7) {
        db.query("INSERT INTO internship_list SET ?", {studentID: num, company: p1, work_day: p2, start_date: startdate, end_date: enddate, text_area: p7}, (error, results) => {
          if(error) {
            console.log(error);
          } else{
            return res.redirect("/studentpage");
        ...
        db.query("SELECT holiday_date FROM holiday_list", async (error, rows, results) => {
          ...
          res.status(200).render('calendar/internshipselect', {
            error_message: 'Sistem belirlemiş olduğunuz staj tarihlerinin, iş günü sayısı ile uyumlu olmadığını belirledi. Aşağı kutuya lütfen neden belirtiniz.',
            "days": days
          });
        });
      }
...
exports.excel = function (req, res) {
  ...
  db.query(query, async (error, rows, results) => {
    ...
    worksheet.columns = [
      { header: 'Sıra No', key: 'count'},
      { header: 'Öğrencinin Adı Soyadı', key: 'name', width: 25 },
      { header: 'T.C. Kimlik No', width: 25 },
      { header: 'Doğum Tarihi', width: 25 },
      { header: 'Okul No', key: 'studentID', width: 25 },
      { header: 'Program Adı', width: 25 },
      { header: 'GSS VAR/YOK', width: 25 },
      { header: 'Staj Başlangıç Tarihi', key: 'start_date', width: 25, style: { numFmt: 'yyyy-mm-dd' } },
      { header: 'Staj Bitiş Tarihi', key: 'end_date', width: 25, style: { numFmt: 'yyyy-mm-dd' } },
      { header: 'Sigorta Başlangıç Tarihi', width: 25 },
      { header: 'Staj Yaptığı Gün Sayısı', key: 'work_day', width: 25 },
      { header: 'Devam Etmediği Günler', width: 25 },
      { header: 'Sayılan tatil günleri', width: 25 },
      { header: 'İşletme Adı', key: 'company', width: 25 },
      { header: 'İşletmede Çalışan Personel Sayısı', width: 25 },
      { header: 'İşletme Tel', width: 25 },
      { header: 'İşletme Adresi', width: 25 },
      { header: 'Talep Edilen Devlet Katkısı Tutarı', width: 25 }
    ]
