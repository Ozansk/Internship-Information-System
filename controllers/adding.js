...
//  var sql = ["SELECT * FROM users WHERE  id LIKE '" + '%' + text + '%' + "' ", "SELECT * FROM users WHERE  name = ?", "SELECT * FROM users WHERE  email = ?"];
  var sql = ["SELECT * FROM users WHERE id LIKE '" + '%' + text + '%' + "' OR name LIKE '" + '%' + text + '%' + "' OR email LIKE '" + '%' + text + '%' + "'"];

db.query(sql[0], [text], async (error, rows, results) => {
